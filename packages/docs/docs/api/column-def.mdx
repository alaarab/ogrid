...
Extends `IColumnMeta` with rendering, editing, and data access properties. The `renderCell` and `cellStyle` properties are React-specific additions; the vanilla JS package uses an equivalent `renderCell` callback that receives a DOM container instead of returning JSX.
