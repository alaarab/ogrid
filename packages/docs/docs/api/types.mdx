...
Reference for all shared types exported from `@alaarab/ogrid-core`. These types are also re-exported from all UI packages (`@alaarab/ogrid`, `@alaarab/ogrid-fluent`, `@alaarab/ogrid-material`).
...
A single filter value. The shape depends on the filter type.

```typescript
type FilterValue = string | string[] | UserLike;
```

| Filter Type | FilterValue Shape | Example |
|------------|-------------------|---------|
| `text` | `string` | `"Alice"` |
| `multiSelect` | `string[]` | `["Active", "Pending"]` |
| `people` | `UserLike` | `{ displayName: "Alice", email: "alice@example.com" }` |
...
const filters: IFilters = {
  ...
  status: ['Active', 'Pending'],              // multiSelect filter
  owner: { displayName: 'Bob', email: 'bob@example.com' }, // people filter
...
import type { IColumnDef, IOGridApi } from '@alaarab/ogrid';
import type { IColumnDef, IOGridApi } from '@alaarab/ogrid-fluent';
import type { IColumnDef, IOGridApi } from '@alaarab/ogrid-material';
```
