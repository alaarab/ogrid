...
:::tip UI styling
The live demo above shows **Radix UI** styling (lightweight default). The same feature renders through the Fluent UI and Material UI packages with their respective native components, plus the `@alaarab/ogrid-js` vanilla theme.
:::
