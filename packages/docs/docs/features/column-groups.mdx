...
const columns: (IColumnDef<Employee> | IColumnGroupDef<Employee>)[] = [
  {
    ...
    children: [
      { columnId: 'firstName', name: 'First Name' },
      { columnId: 'lastName', name: 'Last Name' },
    ],
  },
  {
...
This renders a two-row header: the first row shows "Personal Info" spanning two columns and "Employment" spanning two columns, with the individual column names in the second row.
