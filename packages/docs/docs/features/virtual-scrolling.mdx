...
description: Render thousands of rows with smooth scrolling by only mounting visible rows in the DOM
---
...
Render tens of thousands of rows without degrading performance. Virtual scrolling only mounts the rows currently visible in the viewport (plus a small overscan buffer), keeping the DOM lightweight regardless of dataset size.
...
```tsx
<OGrid
  virtualScroll={{ rowHeight: 48 }}  // taller rows
  // ...
/>
```
...
```tsx
<OGrid
  virtualScroll={{ rowHeight: 36, overscan: 10 }}
  // ...
/>
```
...
|-------|------|---------|-------------|
| `rowHeight` | `number` | -- | Fixed height of each row in pixels (required). |
| `enabled` | `boolean` | `true` | Whether virtual scrolling is active. Set `false` to disable. |
