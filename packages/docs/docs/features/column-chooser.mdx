...
The live demo above shows **React Radix UI** styling (lightweight default). To see how the column chooser dropdown looks in your framework's design system, click the framework buttons above the demo to open StackBlitz. Each framework renders with its native dropdown and checkbox components, so the styling matches your design system.
:::
