...
OGrid ships React UI packages plus a vanilla JS package, and not every browser check needs to run on every push. The browser contract is split into two layers:
