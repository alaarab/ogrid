/**
 * createGridFocus (Angular) — headless arrow-key cell navigation.
 */

import { signal, type WritableSignal } from '@angular/core';
import type { CellCoord, RangeSelectionResult } from './range-selection';

export interface CreateGridFocusParams {
  rowCount: number;
  colCount: number;
  pageSize?: number;
...
export interface GridFocusResult {
  activeCell: WritableSignal<CellCoord | null>;
...
export function createGridFocus(params: CreateGridFocusParams): GridFocusResult {
  const { rowCount, colCount, pageSize = 10, rangeSelection } = params;

  const activeCell = signal<CellCoord | null>(null);
  const setActiveCell = (cell: CellCoord | null) => activeCell.set(cell);

  const moveBy = (drow: number, dcol: number, extendRange = false) => {
    if (rowCount <= 0 || colCount <= 0) return;
    const start = activeCell() ?? { row: 0, col: 0 };
  ...
  const moveToRowEnd = () => {
    ...
    activeCell.set({ row, col });
    rangeSelection?.startRange(row, col);
  };

  const moveToStart = () => {
    activeCell.set({ row: 0, col: 0 });
